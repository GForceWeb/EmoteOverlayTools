...
export function createEmoteCube(images: string[], size: number = 150): void {
  ...
  const sides: CubeSide = {
    ...
    left: createCubeSide('left', size, images[3] || images[1] || images[0] || ''),
    top: createCubeSide('top', size, images[4] || images[2] || images[0] || ''),
    bottom: createCubeSide('bottom', size, images[5] || images[3] || images[1] || images[0] || '')
  };
  ...
  // @ts-ignore - GSAP is included via CDN
  gsap.set(sides.back, { transform: `rotateY(180deg) translateZ(${halfSize}px)` });
  // @ts-ignore - GSAP is included via CDN
  gsap.set(sides.right, { transform: `rotateY(90deg) translateZ(${halfSize}px)` });
  // @ts-ignore - GSAP is included via CDN
  gsap.set(sides.left, { transform: `rotateY(-90deg) translateZ(${halfSize}px)` });
  // @ts-ignore - GSAP is included via CDN
  gsap.set(sides.top, { transform: `rotateX(90deg) translateZ(${halfSize}px)` });
  // @ts-ignore - GSAP is included via CDN
  gsap.set(sides.bottom, { transform: `rotateX(-90deg) translateZ(${halfSize}px)` });
...
function createCubeSide(className: string, size: number, imageUrl: string): HTMLElement {
