...
export function createAvatarChoon(images: string[]): void {
    if (images.length < 1) {
...
function startChoonAnimation(avatar: HTMLElement, musicNotes: HTMLElement[], container: HTMLElement): void {
    // @ts-ignore - GSAP is included via CDN
...
function animateMusicNote(note: HTMLElement, avatar: HTMLElement, delay: number): void {
    ...
    const pathX = startX + (Math.random() < 0.5 ? -1 : 1) * (50 + Math.random() * 100);
