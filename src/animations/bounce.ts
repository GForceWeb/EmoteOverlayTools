...
export function emoteBounce(images: string[], count: number = 5, interval: number = 100): void {
...
function createEmoteBounce(image: string): void {
    ...
    gsap.set(Div, { className: 'bounce-element', x: helpers.Randomizer(0, innerWidth), y: -300, z: helpers.Randomizer(-200, 200), backgroundImage: 'url(' + image + ')' });
...
function bounce_animation(element: HTMLElement): void {
    // @ts-ignore - GSAP is included via CDN
    gsap.to(element, 0.6, { y: innerHeight - 100, ease: Bounce.easeOut, delay: 1 });
    // @ts-ignore - GSAP is included via CDN
    gsap.to(element, 5, { x: '+=' + helpers.Randomizer(-500, 500), delay: 1.6 });
    // @ts-ignore - GSAP is included via CDN
    gsap.to(element, 5, { rotationY: 360, rotationX: 360, repeat: 1, delay: 1.6 });
    // @ts-ignore - GSAP is included via CDN
