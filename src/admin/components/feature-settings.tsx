...
interface FeatureSettingsProps {
  ...
  onPreview: (feature: string) => void
}

export function FeatureSettings({ settings, setSettings, onPreview }: FeatureSettingsProps) {
  const handleFeatureToggle = (feature: keyof Settings["features"], enabled: boolean) => {
    setSettings((prev) => ({
      ...prev,
      features: {
        ...prev.features,
  ...
  return (
    <>
      <CardHeader>
        ...
        <CardDescription>Enable or disable specific features for your stream overlay</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Enable All Features Toggle */}
        <div className="flex items-center justify-between p-4 bg-secondary/50 rounded-lg">
          <div className="space-y-0.5">
            <Label htmlFor="enableAllFeatures" className="text-base font-medium">
              Enable All Features
            </Label>
            <p className="text-sm text-muted-foreground">Turn on all available features at once</p>
          </div>
          <Switch
            ...
            <div key={feature} className="flex flex-col space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Button variant="outline" size="sm" onClick={() => onPreview(feature)}>
                    Preview
                  </Button>
                  <Switch
                    id={`feature-${feature}`}
                    checked={settings.enableAllFeatures ? true : enabled}
                    onCheckedChange={(checked) => handleFeatureToggle(feature as keyof Settings["features"], checked)}
                    disabled={settings.enableAllFeatures}
                  />
                </div>
              </div>
              {feature !== Object.keys(settings.features).pop() && <Separator className="my-2" />}
            </div>
          ))}
        </div>
      </CardContent>
